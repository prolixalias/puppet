module Puppet
  Type.newtype(:schedule_precise) do
    @doc = <<-'EOT'
      ...
      Agent's `statettl` setting affects the ability of a schedule to determine
      if a resource has already been checked. If the `statettl` is set lower
      than the span of the associated schedule resource, then a resource could
      be checked & applied multiple times in the schedule as the information
      about when the resource was last checked will have expired from the cache.
      For instance, to utilize `quarterly` period, a `statettl` setting of "96d"
      (minimum) would be needed on the agent.
      EOT
    ...
    newparam(:period) do
      ...
      newvalues(:hourly, :daily, :weekly, :monthly, :quarterly, :never)
      ...
      ScheduleScales = {
        ...
        :weekly => 604800,
        #:monthly => 2592000,
        :monthly => proc do |prev, now|
          # number of days in month
          #Time.days_in_month(now.month, now.year) * 24
          #((date2.to_time - date1.to_time)/1.month.second).to_i
          #((now - prev).to_f / 365 * 12).round
          #Time.days_in_month(now.month, now.year)
          puts "here is is: #{Time.days_in_month(Time.now.month, Time.now.year)}"
        end,
        :quarterly => proc do |prev, now|
          # number of days in month
          Time.days_in_month(now.month, now.year) * 24
        end
      }
